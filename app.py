...
def build_client_mini_report_ai_prompt() -> str:
    return """
Ты — эксперт по Системе Потенциалов Человека (СПЧ), школа «НеоСознание».
Ты делаешь КЛИЕНТСКИЙ мини-отчёт, который:
— узнаваемый,
— точный,
— производит эффект «как это вообще возможно так точно описать меня».

⚠️ ЖЁСТКИЕ ЗАПРЕТЫ (НАРУШЕНИЯ НЕДОПУСТИМЫ):
- ❌ НЕ выдумывай свойства потенциалов.
- ❌ НЕ используй матрицу 3×3 для описания позиций 4–6.
- ❌ НЕ смешивай 1–3 и 4–6.
- ❌ НЕ додумывай смыслы, которых нет в canon_texts.
- ❌ НЕ используй эзотерику, терапию, диагнозы, практики, советы.
- ❌ НЕ меняй логику школы.

РАЗРЕШЕНО:
- Перефразирование канона.
- Связки между позициями.
- Ясные, сильные формулировки.
- Эффект узнавания.

---

## 🔹 КЛЮЧЕВАЯ ЛОГИКА (ОБЯЗАТЕЛЬНО СОБЛЮДАТЬ)

### Позиции 1–3 — ЭТО ЯДРО ЛИЧНОСТИ
Они:
- описывают КАК человек воспринимает, зачем живёт и через что реализуется
- ВСЕГДА читаются ТОЛЬКО из canon_texts.pos1–pos3
- опираются на матрицу 3×3

### Позиции 4–6 — ЭТО ПРИКЛАДНОЙ СЛОЙ
Они:
- НЕ ЯВЛЯЮТСЯ рядами матрицы
- НЕ читаются из matrix
- описываются ТОЛЬКО из canon_texts.pos4–pos6
- отвечают на вопросы:
  4 — с какой болью приходят
  5 — с какой целью приходят
  6 — какой результат получают

Если ты используешь matrix для 4–6 — это ОШИБКА.
...
Вставь matrix_md БЕЗ ИЗМЕНЕНИЙ.

## Как читать эту матрицу (очень важно)

В системе потенциалов используется правило **60–30–10**.  
Это не теория, а базовый закон устойчивой жизни, энергии и реализации.

### 🔹 1 ряд — Потенциалы силы (60%)
Это твоя **природа, ядро личности и способ реализации**.  
Через эти потенциалы ты:
- воспринимаешь мир,
- принимаешь решения,
- зарабатываешь,
- создаёшь ценность.

Если ты **слишком много** живёшь по 1 ряду — возникает выгорание, давление, болезни, финансовые откаты.  
Если **слишком мало** — апатия, застой, ощущение «я не на своём месте».

### 🔹 2 ряд — Потенциалы наполнения (30%)
Это твой **внутренний бензин**.  
Через них ты:
- отдыхаешь,
- вдохновляешься,
- наполняешься энергией,
- восстанавливаешь интерес к жизни.

⚠️ Важно: потенциалы 2 ряда **наполняют потенциалы 1 ряда**, стоящие над ними.  
Если ты не наполняешься по 2 ряду — 1 ряд начинает «ломаться».

Слишком много 2 ряда → резкий слив энергии, слабость, дыры в доходе.  
Слишком мало → синдром вечной усталости.

### 🔹 3 ряд — Слабые зоны (10%)
Это **не твоя реализация**.  
Здесь:
- быстрее всего происходит слив энергии,
- включается сопротивление,
- появляются ошибки и перегрузки.

Эти потенциалы **нельзя развивать** — их нужно:
- делегировать,
- отдавать партнёрам,
- закрывать системой или другими людьми.

Использовать 3 ряд можно **не более 10% времени**.

---

### 3. ЯДРО ЛИЧНОСТИ — ПОЗИЦИИ 1–3

#### Позиция 1 — {pot1}. Восприятие
Используй ТОЛЬКО canon_texts.pos1  
6–10 строк.  
Как человек:
- воспринимает мир
- анализирует
- принимает решения
- чувствует «верно / не верно»

#### Позиция 2 — {pot2}. Мотивация и образ
Используй ТОЛЬКО canon_texts.pos2  
6–10 строк.  
Про:
- зачем он действует
- что его зажигает
- каким его считывают другие
- через что приходит признание

#### Позиция 3 — {pot3}. Инструмент реализации
Используй ТОЛЬКО canon_texts.pos3  
6–10 строк.  
Про:
- что он ДЕЛАЕТ
- через какие действия достигает целей
- чем реально полезен другим

---

### 4. ПРИКЛАДНОЙ СЛОЙ — ПОЗИЦИИ 4–6  
⚠️ НЕ ИСПОЛЬЗУЙ МАТРИЦУ

#### Позиция 4 — {pot4}. Боль и сфера анализа
Используй ТОЛЬКО canon_texts.pos4

Структура:
- С какой проблемой к тебе приходят люди (маркер-лист)
- Какую именно проблему тебе интересно анализировать
- Хобби восприятия (как ты отдыхаешь от анализа)

Если данных нет →  
«Нужно уточнение: какую проблему ты чаще всего разбираешь у людей?»

---

#### Позиция 5 — {pot5}. Цель и запрос клиентов
Используй ТОЛЬКО canon_texts.pos5

Структура:
- С какой целью приходят люди (маркер-лист)
- Какую миссию/цель закрывает твой продукт
- Творческое хобби (в одиночестве)

Если данных нет →  
«Нужно уточнение: ради какой цели к тебе приходят клиенты?»

---

#### Позиция 6 — {pot6}. Результат
Используй ТОЛЬКО canon_texts.pos6

Структура:
- Какой результат получают люди
- Что у них появляется после взаимодействия с тобой
- Коллективное хобби (с людьми)

Если данных нет →  
«Нужно уточнение: какой результат люди чаще всего получают от работы с тобой?»

---

### 5. СВЯЗКА 4 → 5 → 6 (ОБЯЗАТЕЛЬНО)
6–10 строк:

- как боль (4) превращается в цель (5)
- как цель приводит к результату (6)
- как ядро (1–3) делает это возможным
- финал:  
  «Полный мастер-разбор показывает, как усилить эту связку и убрать слабые места»
...
def build_client_mini_report_ai(client, model: str, payload: dict) -> str:
    ...
    name = _s(meta.get("name")) or "Ты"
    request = _s(meta.get("request")) or "твой запрос"
...
def build_client_report_prompt():
    return """
Ты — мастер СПЧ. Сделай КЛИЕНТСКИЙ МИНИ-ОТЧЁТ.
Ты ОБЯЗАН использовать ТОЛЬКО переданные canon_texts.
Запрещено додумывать свойства потенциалов.

ЖЁСТКО:
- Пиши по-русски
- Используй названия камней
- Не описывай ряды (1/2/3) как смысловые блоки
- Работай ТОЛЬКО с позициями 1–6
- Если по позиции текст = "—" или "⚠️" → напиши: “Эта зона требует уточнения на разборе”

ФОРМАТ ВЫВОДА (СТРОГО):

<<<CLIENT_REPORT>>>

# Твоя матрица потенциалов 3×3 (предварительный отчёт)
Запрос клиента: <request>

## Матрица 3×3
<вставь matrix_3x3_md без изменений>
...
## 4 потенциал — Сфера анализа / проблема клиента
Камень: <pos4>
<canon_texts.pos4>

---

## 5 потенциал — Миссия / запрос людей
Камень: <pos5>
<canon_texts.pos5>

---

## 6 потенциал — Результат, который ты даёшь
Камень: <pos6>
<canon_texts.pos6>

---

Короткий вывод (3–5 строк):
— В чём твоя ключевая сила
— С чем к тебе приходят
— Почему тебе важно работать именно так
