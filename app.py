...
KEYWORDS = {
    # 7 — Янтарь (порядок/систематизация/автоматизация/механизмы/учёт)
    "Янтарь": [
        "порядок","структур","регламент","документ","учет","учёт","бухгалтер",
        "систем","системат","автоматизац","процесс","процедур","инструкц","чеклист", "IT",
        "таблиц","эксель","excel","crm","схем","алгоритм","механизм","починк","чинить",
        "настроить","настройк","оптимизац","администр","контроль качества","аудит",
        "комфорт","уют","планиров","логистик",  # янтарь как “организация быта/механики”
        "медицина","здоров","кропотлив","внимание к детал","перфекц",  # из описания 3 ряда
    ],

    # 9 — Шунгит (тело/нагрузки/выносливость/монотонная физ.работа/походы)
    "Шунгит": [
        "тело","спорт","движ","трен","физ","вынослив","сила","силов","мышц",
        "нагрузк","пульс","кардио","бег","ходьб","шаг","поход","длительн",
        "производств","ручн","монотонн","рутинн физ","тяжел","устал","восстанов",
        "здоровье","режим","сон",
    ],

    # 3 — Цитрин (действия/привычки/деньги/результат/продажи/управление)
    "Цитрин": [
        "деньг","доход","прибыл","выручк","результат","эффектив","быстро","скорост",
        "выгод","цифр","метрик","kpi","план продаж","воронк","продаж","маржин",
        "управлен действ","привычк","дисциплин","задач","дедлайн","срок",
        "организ","операционк","процесс продаж","команда по местам","распредел",
        "менеджмент","управлен персонал","контроль задач","пинок","ускор",
        "бизнес","рост","масштаб","монетиз","коммерц","сделк", "танц", "гибк", "растяж", "перв", "лучш",
    ],

    # 6 — Гранат (эмоции/популярность/признание/театр/публичность)
    "Гранат": [
        "популяр","признан","внимание","харизм","эмоц","чувств","театр","драм",
        "актёр","игра","сцена","шоу","праздник","вечерин","ивент","мероприят",
        "мимик","жест","подач","вкусно рассказ","сторис","рилс","reels","видео", "туса",
        "соцсет","аудитор","люди вокруг","впечатлить","ярко","красиво показать",
    ],

    # 4 — Сапфир (смысл/идеи/духовность/философия/поиск пути)
    "Сапфир": [
        "смысл","идея","почему","глубин","философ","духов","ценност","путь",
        "поиск","предназнач","вера","внутр","ответы","осознан","медитац",
        "тишин","созерц","астроном","теори","концепц","идеолог",
        "поющие чаши","звук природы","флоатинг", "муз",
    ],

    # 5 — Гелиодор (знания + продвижение знаний/популяризация/сообщество)
    "Гелиодор": [
        "уч","обуч","знан","курс","урок","объясн","настав","разбор","метод",
        "педагог","препод","структурирую знание","тренинг","семинар",
        "продвиж","популяриз","известн","публичн","инфосообществ","сообщество", "связи", "нетворк",
        "идея","идеолог","эксперт","контент","лекц", "готовить", "вкус", "кушать", "Рецепт", "еда", "ресторан", "знакомств",
        "вокал","петь","караоке",  # из твоей страницы “что нравится”
    ],

    # 2 — Изумруд (гармония/красота/эстетика/уют/эмоциональная лёгкость)
    "Изумруд": [
        "красот","эстет","уют","дизайн","атмосфер","стиль","гармони",
        "внешност","фото","образ","наряд","макияж","накрас","украш",
        "дом","интерьер","картинк","красиво","приятно", "чувств",
        "эмоционал гармони","легкост","интерес к жизни","краски жизни","eq",
        "психолог","душевн","тёплое общение",
    ],

    # 8 — Рубин (адреналин/риск/съёмки/режиссура/впечатления/эмо-мероприятия)
    "Рубин": [
        "драйв","адренал","риск","приключ","впечат","эмоц","экстрим",
        "сцена","выступ","шоу","ивент","мероприят", "секс",
        "режисс","съёмк","снимать","видеоролик","контент","клип","камера",
        "организация мероприятий","эмоциональн мероприят",
    ],

    # 1 — Аметист (мышление/анализ/интуиция/цель/время/упаковка идеи)
    "Аметист": [
        "цель","стратег","вектор","план","управлен","лидер","проект",
        "анализ","логик","мышлен","вариант","решени","эффективност",
        "предвид","интуиц","яснознан","первая мысль","сразу понял", "управл", 
        "упаков","преподнести","сформулировать","собрать в систему", "нумеролог", "эзотерик", "мистик",
        "подсозн","убежден","перепрош","перестроить мышление", "логик", "тайн", "астролог",
        "время","тайминг","точность", "знать", "знаю", "чувств", "мысл", "миссия", "Интуиц", "созн",
    ],
}
...
def score_all(answers: dict):
    ...
    bump_if("childhood.child_play", {
        "Командовать/организовывать":"Аметист",
        "Учиться/читать/объяснять":"Гелиодор",
        "Выступать/быть заметным(ой)":"Рубин",
        "Дружить/общаться/мирить":"Гранат",
        "Рисовать/украшать/делать красиво":"Изумруд",
