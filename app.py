...
def question_plan():
    ...
    return [
        ...
        {"id": "intake.priority_area", "stage": "intake", "intent": "priority_area", "type": "single",
         "text": "Что важнее всего прояснить сегодня? (выбери быстро)",
        ...
        {"id": "now.easy_tasks", "stage": "now", "intent": "easy_tasks", "type": "text",
         "text": "Что у тебя реально получается легко и быстро (как будто само)? 3 примера."},
        {"id": "now.praise_for", "stage": "now", "intent": "praise_for", "type": "text",
         "text": "За что тебя чаще всего хвалят/просят помочь? (1–3 пункта)"},
        {"id": "now.time_flow", "stage": "now", "intent": "time_flow", "type": "text",
         "text": "В какой деятельности ты теряешь счёт времени? (что делаешь и почему)"},
        ...
        {"id": "now.energy_fill", "stage": "now", "intent": "energy_fill", "type": "multi",
         "text": "Что тебя реально наполняет? Выбери 1–4 (как есть, не «правильно»).",
        ...
        {"id": "now.best_result_example", "stage": "now", "intent": "best_result_example", "type": "text",
         "text": "Дай 1 конкретный пример: ситуация → что ты сделал(а) → результат (то, что у тебя получается лучше большинства)."},
        {"id": "now.motivation_trigger", "stage": "now", "intent": "motivation_trigger", "type": "single",
         "text": "Что сильнее всего тебя включает (первая реакция)?",
         "options": ["Цель/стратегия/вектор", "Люди/связь/влияние", "Красота/уют/эстетика",
                     "Смысл/идея/глубина", "Драйв/сцена/эмоции", "Деньги/результат/скорость"]},

        # =========================
        # 2) SCENARIOS (первые 3 секунды)
        # =========================
        {"id": "scn.new_space_first", "stage": "scenarios", "intent": "new_space_first", "type": "single",
         "text": "Представь: ты заходишь в новое пространство (встреча/мероприятие/дом). Что ты замечаешь первым — автоматически?",
         "options": ["Людей/эмоции", "Смысл/идею/зачем", "Деньги/возможности/выгоду", "Порядок/хаос/правила", "Красоту/атмосферу/детали"]},
        ...
        {"id": "scn.decide_fast", "stage": "scenarios", "intent": "decide_fast", "type": "single",
         "text": "Нужно принять решение быстро. Ты чаще опираешься на…",
         "options": ["Цифры/выгоду", "Интуицию/чувство", "Смысл/ценности", "Людей/отношения", "Правила/порядок"]},
        ...
        {"id": "behavior.group_role_now", "stage": "behavior", "intent": "group_role_now", "type": "single",
         ...
         "options": ["Объединяю людей", "Продавливаю результат", "Придумываю смысл/идею",
                     "Структурирую/наводжу порядок", "Делаю красиво/атмосферу", "Вдохновляю/зажигаю"]},
...
KEYWORDS = {
    "Янтарь": ["порядок","структур","регламент","документ","система","учет","организац","процесс"],
    "Шунгит": ["тело","спорт","движ","трен","физ","вынослив"],
    "Цитрин": ["деньг","доход","результат","быстро","выгода","цифр","эффектив","продаж"],
    "Изумруд": ["красот","эстет","уют","дизайн","атмосфер","стиль","гармони"],
    "Рубин": ["драйв","сцена","ивент","впечат","приключ","эмоц","адренал","выступ"],
    "Гранат": ["люд","команд","общен","поддерж","забот","отношен","объедин","нетворк"],
    "Сапфир": ["смысл","идея","почему","глубин","философ","концепц","ценност"],
    "Гелиодор": ["уч","обуч","знан","курс","объясн","настав","разбор","метод"],
    "Аметист": ["цель","стратег","вектор","управлен","лидер","проект","план","организовать"],
}
...
def score_all(answers: dict):
    ...
    def bump(qid, mapping, amt=1.0):
        ...
        if isinstance(a, list):
            for x in a:
                p = mapping.get(x)
                if p:
                    scores[p] += amt / max(1, len(a))
        else:
            p = mapping.get(a)
            if p:
                scores[p] += amt

    bump("intake.priority_area", {
    ...
    bump("childhood.child_play", {
        ...
        "Бегать/соревноваться/движение":"Шунгит",
    }, amt=0.8)

    bump("behavior.money_spend", {
