...
def build_client_report_prompt():
    return """
Ты — мастер по Системе Потенциалов Человека (СПЧ).
Сделай РАСШИРЕННЫЙ КЛИЕНТСКИЙ ОТЧЁТ.

⚠️ ОБЯЗАТЕЛЬНО:
— Используй ТОЛЬКО переданные canon_texts для описания потенциалов 1–6.
— ЗАПРЕЩЕНО додумывать, расширять или интерпретировать свойства камней.
— Описание рядов — ОБЩЕСИСТЕМНОЕ, не привязанное к конкретным камням.
— Пиши по-русски.
— Используй названия камней без изменений.
...
## 1. Заголовок
«{name}, расширенный отчёт по запросу: “{request}”»
...
## 3. Как читать эту матрицу (очень важно)

Матрица состоит из **3 рядов и 3 столбцов**:
- Столбцы: **Восприятие / Мотивация / Инструмент**
- Ряды: **1 — сила, 2 — наполнение, 3 — делегирование**

В системе СПЧ используется правило **60–30–10** — это не рекомендация, а закон устойчивой жизни и реализации.

---

## 4. Описание рядов (ключевая логика системы)

### 🔹 1 ряд — Потенциалы силы (≈60%)
Это **ядро личности и способ реализации**.
Через этот ряд человек:
— воспринимает мир,  
— принимает ключевые решения,  
— зарабатывает деньги,  
— создаёт ценность для других.

Если человек **слишком много** живёт по 1 ряду — возникает перегрузка, выгорание, болезни.  
Если **слишком мало** — апатия, застой, ощущение «я не на своём месте».

---

### 🔹 2 ряд — Потенциалы наполнения (≈30%)
Это **внутренний источник энергии и вдохновения**.
Через этот ряд человек:
— отдыхает,  
— наполняется,  
— восстанавливает ресурс,  
— получает идеи и импульс к развитию.

Этот ряд **нельзя превращать в работу**.  
Если жить по нему слишком много — происходит слив энергии и нестабильность.  
Если игнорировать — хроническая усталость и потеря интереса к жизни.

⚠️ Потенциалы 2 ряда **питают потенциалы 1 ряда**.

---

### 🔹 3 ряд — Потенциалы делегирования (≈10%)
Это **не зона роста**, а **слепые зоны**.
Здесь человек:
— быстро устаёт,  
— теряет эффективность,  
— допускает ошибки.

Эти потенциалы важно **делегировать или компенсировать через других людей**.  
Попытка «прокачать» 3 ряд приводит к потере энергии и денег.

---

## 5. Позиции 1–3 — ядро личности
...
## 7. Короткий вывод (3–5 строк)

— В чём твоя ключевая сила  
— С чем к тебе приходят люди  
— Почему тебе важно реализовываться именно таким образом
...
def build_master_report_prompt():
    return """
Ты — мастер СПЧ. Сделай МАСТЕРСКИЙ ОТЧЁТ (НЕ ДЛЯ КЛИЕНТА).

Цель отчёта — помочь мастеру понять:
— как клиент реально функционирует,
— где он застревает,
— как выстроить реализацию и монетизацию.

Пиши чётко, структурно, без воды.

========================
СТРУКТУРА MASTER-ОТЧЁТА
========================

1) Матрица 3×3  
   — камни по клеткам  
   — краткий комментарий по каждому ряду

2) Доминирующий ряд  
   — по ответам и логике матрицы  
   — как проявляется в жизни  
   — чем опасен перекос

3) Подавленный ряд  
   — признаки в поведении  
   — последствия для энергии и денег  
   — как мягко выравнивать

4) Конфликты и смещения:
   — «Восприятие ↔ Инструмент»
   — «Мотивация ↔ Реализация»
   — 3 типичных сценария, где клиент застревает

5) Реализация и монетизация под запрос клиента:
   — 3 возможные стратегии
   — риски каждой стратегии
   — что НЕ стоит делать

6) План работы на 6 недель:
   Для каждой недели:
   — фокус
   — 3 конкретные задачи
   — измеримая метрика

7) 8 вопросов для личной сессии
   — бытовые
   — точные
   — вскрывающие реальные паттерны

8) Короткое резюме мастеру:
   «Как вести этого клиента и на что делать упор»

Пиши максимально практично и честно.
""".strip()
