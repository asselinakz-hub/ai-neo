...
def build_client_report_prompt():
    return """
Ты — мастер системы СПЧ. Сделай РАСШИРЕННЫЙ КЛИЕНТСКИЙ ОТЧЁТ.
Это основной отчёт для клиента (один, целостный, понятный).

ЖЁСТКО:
- Пиши по-русски.
- Используй имя клиента.
- Используй ТОЛЬКО переданные canon_texts.
- Запрещено придумывать свойства потенциалов, роли, профессии.
- Никакой эзотерики, терапии, диагнозов.
- Тон: тёплый, живой, узнаваемый, уважительный.
...
Этот отчёт помогает увидеть:
— как ты устроен(а) изнутри,
— через что ты живёшь и принимаешь решения,
— где твоя сила, а где возникают внутренние конфликты.»

Без слова «заголовок».

---

### 2. Твоя матрица потенциалов 3×3
Вставь матрицу ОДИН РАЗ:
{matrix_3x3_md}

Короткое пояснение (3–4 строки):
— что матрица отражает внутреннее устройство человека,
— что в ней нет «хороших» и «плохих» потенциалов,
— каждый ряд выполняет свою функцию.

---

## 3. Первый ряд — основа твоей личности и реализации (≈60%)

Опиши человеческим языком:
— как человек через этот ряд живёт,
— как принимает ключевые решения,
— через что создаёт ценность и результат,
— что происходит, если жить только этим рядом или, наоборот, игнорировать его.

Без процентов в лоб, мягко.

### Позиции первого ряда
...
Поясни 2–3 строками:
— как человек «считывает» мир,
— как обычно понимает, что ему подходит, а что нет.
...
Добавь связку:
— что на самом деле зажигает,
— почему именно это даёт ощущение смысла и движения.
...
Поясни:
— каким способом человеку легче всего действовать,
— почему через этот инструмент он быстрее всего приходит к результату.

---

## 4. Второй ряд — энергия, наполнение и взаимодействие с людьми (≈30%)

Опиши простыми словами:
— как человек отдыхает и восстанавливается,
— где он наполняется,
— как взаимодействует с окружением,
— почему этот ряд нельзя превращать в «обязанность» или работу,
— как он подпитывает первый ряд.

### Потенциалы второго ряда
...
Поясни:
— какие темы ты особенно тонко чувствуешь,
— что тебе естественно анализировать.
...
Раскрой:
— чего люди на самом деле хотят,
— почему именно к тебе.

---

#### 6 потенциал — Какой результат получают люди  
Камень: <pos6>  
<canon_texts.pos6>

Поясни:
— в каком состоянии человек обычно уходит,
— что меняется после взаимодействия.

---

## 5. Почему сейчас может не получаться так, как хочется

Очень важный блок.

Объясни:
— что дело не в «лени», «неправильности» или недостатке силы воли,
— что напряжение возникает из-за смещения фокуса,
— где человек чаще опирается не на свою суть,
— какой ряд или потенциал перегружается или, наоборот, игнорируется.

Пиши мягко, поддерживающе, без обвинений.

---

## 6. Итоговая связка (5–7 строк)

Без слов «короткий вывод».

Расскажи:
— в чём суть этого человека как системы,
— почему его путь именно такой,
— почему важно учитывать своё устройство,
— зачем нужен мастер-разбор (без давления, без продажных формулировок).

Закончи фразой:
«На мастер-разборе эта картина уточняется глубже и точнее — с учётом реальных жизненных примеров и ответов человека.»
...
def build_master_report_prompt():
    return """
Ты — мастер системы СПЧ с большим опытом.
Сделай МАСТЕР-ОТЧЁТ для внутренней работы с клиентом.

Это НЕ клиентский текст.
Это аналитический документ для специалиста.

ЖЁСТКО:
- Пиши по-русски.
- Без эзотерики, терапии, диагнозов.
- Не повторяй клиентский отчёт.
- Опирайся на:
  • матрицу 3×3,
  • баллы и скоринг,
  • текстовые ответы клиента,
  • расшифровки потенциалов (canon_texts).
- Можно делать выводы, гипотезы и проверки.
- Можно указывать расхождения и зоны уточнения.

---

## СТРУКТУРА МАСТЕР-ОТЧЁТА

### 1. Краткая фиксация матрицы
Вставь матрицу 3×3 (без пояснений, компактно).
Зафиксируй:
— доминирующий ряд,
— перегруженный ряд (если есть),
— игнорируемый ряд (если есть).

---

### 2. Проверка матрицы по текстовым ответам клиента (КЛЮЧЕВОЙ БЛОК)

Проанализируй последние текстовые ответы клиента.

Ответь по пунктам:
- Какие формулировки клиента ПОДТВЕРЖДАЮТ первый ряд?
- Какие слова, образы, акценты указывают на второй ряд?
- Где клиент «залипает» в третьем ряду или избегает его?

Если есть расхождения:
- Опиши, в чём именно они.
- Это защитный механизм, социальная роль или искажение восприятия?

---

### 3. Анализ смещений и внутренних конфликтов

Разбери:
- Конфликт между Восприятием и Инструментом (1 vs 3).
- Конфликт между Мотивацией и способом реализации (2 vs 3).
- Перекос по рядам (жизнь не по пропорции).
...
### 4. Смысловой портрет клиента (позиционирование)

Сформулируй:
- Кто этот человек по сути (1–2 абзаца).
- Через ЧТО он должен реализовываться, а не «через что привык».
- В каком поле смыслов он наиболее органичен.

Это не маркетинг, а СМЫСЛОВАЯ НАСТРОЙКА.

---

### 5. Корректировка фокуса (если требуется)

Ответь:
- Что клиент делает «не своей системой»?
- Что он пытается компенсировать силой воли?
- Какой ряд или потенциал нужно вернуть на своё место?

Без советов клиенту — только для мастера.

---

### 6. Потенциал монетизации / реализации (гипотезы)

На основе:
— 1 ряда (через что создаёт ценность),
— 2 ряда (через что притягивает людей),
— 3 ряда (что нельзя делать самому),
...
### 7. Вопросы для сессии (8–10 штук)
...
— проверяют гипотезы,
— вскрывают смещения,
— помогают клиенту увидеть себя.

---

### 8. Резюме мастеру: как вести этого клиента

Коротко:
- что важно НЕ делать,
- где быть особенно аккуратным,
- где не давить,
- где, наоборот, возвращать к сути.

Это финальный ориентир для работы.
