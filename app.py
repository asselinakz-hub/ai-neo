...
# ======================
# CANON PROFILES (from "Позиции потенциалов.docx")
# Используем ТОЛЬКО для описаний в отчёте (не для распознавания).
# ======================

POT_CANON = {
    "Сапфир": {
        "perception": {
            "title": "Сапфир — восприятие (звук / вибрация идеи)",
            "text": (
                "Ты воспринимаешь мир через звук и «вибрацию смысла»: улавливаешь, работает ли идея/убеждение, "
                "где мысль дисгармонична и что в ней «не звучит». Важны тишина и внутреннее прислушивание. "
                "Сильная память на услышанное, тяга к пониманию и осмыслению."
            )
        },
        "motivation": {
            "title": "Сапфир — мотивация (создание смыслов)",
            "text": (
                "Тебя включает создание новых смыслов, идей, концепций, расширение мышления и поиск ответа. "
                "Внутренний драйвер — «понять», «осознать», «собрать целостную картину»."
            )
        },
        "instrument": {
            "title": "Сапфир — инструмент (смысловое изобретение)",
            "text": (
                "Твой инструмент — систематизация идей и рождение новых смыслов: ты умеешь слушать запрос, "
                "видеть недостающий пазл в мышлении и выдавать ясность, концепт, объяснение."
            )
        }
    },
    ...
    "Гелиодор": {
        "perception": {
            "title": "Гелиодор — восприятие (вкус идеи / диалог)",
            "text": (
                "Ты считываешь «вкус идеи»: насколько она интересна людям, смешно/не смешно, зайдёт/не зайдёт. "
                "Восприятие включается через диалог, спор, обмен мнениями и проговаривание."
            )
        },
        "motivation": {
            "title": "Гелиодор — мотивация (популярность смыслов)",
            "text": (
                "Тебя мотивирует признание и распространение идей: рост охватов, интерес аудитории, сила голоса, "
                "умение рассказывать и вовлекать. Когда ты «вещаешь», энергия растёт."
            )
        },
        "instrument": {
            "title": "Гелиодор — инструмент (продвижение знаний голосом)",
            "text": (
                "Твой инструмент — голос и продвижение смыслов: проговаривать, объяснять, импровизировать, "
                "делать идеи понятными и желанными для людей, усиливать популярность через подачу."
            )
        }
    },
    ...
    "Аметист": {
        "perception": {
            "title": "Аметист — восприятие (запах / яснознание)",
            "text": (
                "Ты воспринимаешь через «яснознание»: чувствуешь направление и исход, считываешь эффективность "
                "идеи/мышления для цели и времени. Видишь сценарии решений и к чему они приведут."
            )
        },
        "motivation": {
            "title": "Аметист — мотивация (тайное знание / власть над мышлением)",
            "text": (
                "Тебя зажигают знания, смыслы и скрытые механизмы мышления: понять то, что не видят другие, "
                "влиять на траекторию решений, раскрывать суть и управлять вниманием."
            )
        },
        "instrument": {
            "title": "Аметист — инструмент (управление мышлением / сценариями)",
            "text": (
                "Твой инструмент — работа со сценариями мышления: слова, смыслы, продажи идей, развязка "
                "узлов в убеждениях, настройка мышления под цель и результат."
            )
        }
    },
    ...
    "Изумруд": {
        "perception": {
            "title": "Изумруд — восприятие (визуал + переживание эмоций)",
            "text": (
                "Ты воспринимаешь через визуальные образы и эмоциональное переживание: считываешь красоту, "
                "гармонию эмоций и дисбаланс. Запоминаешь через «увидеть», чувствуешь настроение как краски."
            )
        },
        "motivation": {
            "title": "Изумруд — мотивация (гармония / красота / эмоциональное здоровье)",
            "text": (
                "Тебя мотивирует создание красоты и эмоциональной гармонии: исцелять состояние, добавлять "
                "краски жизни, делать пространство/образ/ощущения более живыми и цельными."
            )
        },
        "instrument": {
            "title": "Изумруд — инструмент (эмоциональное изобретение)",
            "text": (
                "Твой инструмент — создавать и «переливать» эмоции и красоту: атмосфера, визуал, стиль, "
                "состояние, эстетика как способ влияния и результата."
            )
        }
    },
    ...
    "Гранат": {
        "perception": {
            "title": "Гранат — восприятие (мимика / социальная эмоция)",
            "text": (
                "Ты воспринимаешь через мимику, выражение эмоций и отношения: быстро считываешь людей, "
                "дружбу, связь, «свой–чужой», умеешь заражать состоянием и создавать круг коммуникаций."
            )
        },
        "motivation": {
            "title": "Гранат — мотивация (эмоциональная популярность / красота)",
            "text": (
                "Тебя зажигает выражение эмоций, сцены, роли, красота как эмоциональный продукт, желание "
                "быть заметной/вовлекать/влюблять и делать эмоцию трендом."
            )
        },
        "instrument": {
            "title": "Гранат — инструмент (мимика / вовлечение людей)",
            "text": (
                "Твой инструмент — коммуникация через эмоции: роли, мимика, актёрская подача, умение "
                "создавать эмоциональный контакт и удерживать внимание группы."
            )
        }
    },
    ...
    "Рубин": {
        "perception": {
            "title": "Рубин — восприятие (гормональный отклик / адреналин)",
            "text": (
                "Ты воспринимаешь через внутренний гормональный отклик: возбуждает/не возбуждает, даёт драйв "
                "или нет. Чувствуешь силу эмоции телом и её потенциал для продажи/шоу/драмы."
            )
        },
        "motivation": {
            "title": "Рубин — мотивация (охота за эмоциями)",
            "text": (
                "Тебя мотивируют эмоции, адреналин, новизна впечатлений и управление эмоциональными всплесками — "
                "создать сильную реакцию и проживание."
            )
        },
        "instrument": {
            "title": "Рубин — инструмент (драматургия / управление эмоциями)",
            "text": (
                "Твой инструмент — режиссура эмоций: управлять драмой, состояниями, реакциями людей и толпы, "
                "строить эмоциональные сценарии и продажи через эмоцию."
            )
        }
    },
    ...
    "Янтарь": {
        "perception": {
            "title": "Янтарь — восприятие (механизм / комфорт–дискомфорт)",
            "text": (
                "Ты воспринимаешь мир как механизм: что работает/не работает, где поломка, где система даёт сбой. "
                "Ориентир — телесный комфорт/дискомфорт, порядок/непорядок."
            )
        },
        "motivation": {
            "title": "Янтарь — мотивация (создание систем и механизмов)",
            "text": (
                "Тебя зажигает собирать систему «как часы»: структура, инструкции, механика, функциональные решения, "
                "где всё разложено по полочкам."
            )
        },
        "instrument": {
            "title": "Янтарь — инструмент (механическое изобретение / руки)",
            "text": (
                "Твой инструмент — создавать/чинить/систематизировать физику: процессы, механизмы, устройства, "
                "структуры; часто через руки и точную настройку деталей."
            )
        }
    },
    ...
    "Шунгит": {
        "perception": {
            "title": "Шунгит — восприятие (форма / плотность / пространство)",
            "text": (
                "Ты воспринимаешь через форму и плотность: тело, пространство, объём, «оригинал–подделка». "
                "Сильное пространственное мышление, ощущение мира телом."
            )
        },
        "motivation": {
            "title": "Шунгит — мотивация (сообщество вокруг формы/физики)",
            "text": (
                "Тебя включает создание/развитие сообщества вокруг телесного или функционального: форма, здоровье, "
                "производство, практические системы, близкий круг."
            )
        },
        "instrument": {
            "title": "Шунгит — инструмент (действие / тело как компас)",
            "text": (
                "Твой инструмент — действие и тело: через движение ты понимаешь истину и выбираешь путь; "
                "в обучении и работе важны практика, повторение и физическое включение."
            )
        }
    },
    ...
    "Цитрин": {
        "perception": {
            "title": "Цитрин — восприятие (эффективность материи / ресурсы)",
            "text": (
                "Ты считываешь материальные ресурсы: деньги, действия, логистику, эффективность вложений и процессов. "
                "Видишь, где можно упростить и усилить результат."
            )
        },
        "motivation": {
            "title": "Цитрин — мотивация (материальный рост и изобилие)",
            "text": (
                "Тебя мотивируют деньги, рост, уровень, статус, материальные блага и свобода выбора. "
                "Включается азарт результата и повышения планки."
            )
        },
        "instrument": {
            "title": "Цитрин — инструмент (управление действиями и ресурсами)",
            "text": (
                "Твой инструмент — управлять действиями, ресурсами и деньгами: организация процессов, логистика, "
                "привычки, продажи, повышение эффективности и результата."
            )
        }
    },
}

def describe_cell(pot: str, col: str) -> str:
    """
    col: 'perception' | 'motivation' | 'instrument'
    Возвращает каноническое описание ячейки (без фантазии модели).
    """
    block = (POT_CANON.get(pot) or {}).get(col) or {}
    title = block.get("title", pot)
    text = block.get("text", "")
    return f"**{title}**\n\n{text}"
...
# ======================
# CLIENT REPORT (канонический, без путаницы)
# ======================
def build_client_mini_report(payload: dict) -> str:
    meta = payload.get("meta", {}) or {}
    name = meta.get("name") or "Ты"
    request = (meta.get("request") or "твой запрос").strip()

    answers = payload.get("answers", {}) or {}
    scores = payload.get("scores", {}) or {}
    vectors = payload.get("vectors", []) or []

    # Матрица (важно: берём готовую из payload, если ты её туда кладёшь)
    # Если в payload нет — попробуем собрать из scores/col_scores.
    matrix = payload.get("matrix") or {}
    rows = (matrix or {}).get("rows") or []

    # fallback: если матрица не пришла, попробуем собрать здесь
    if not rows:
        try:
            col_scores = payload.get("col_scores") or {}
            # если у тебя новая функция:
    ...
    # текстовые поля “что наполняет / что сливает”
    ...
    avoid = str(answers.get("antipattern.avoid", "") or "").strip() or "—"
    trap = str(answers.get("antipattern.perfection_trap", "") or "").strip() or "—"
    stress = str(answers.get("now.stress_pattern", "") or "").strip() or "—"

    # векторы (без воды)
    vtxt = "\n".join([f"- {v}" for v in vectors[:6]]) if vectors else "- —"

    # Таблица матрицы (красиво)
    matrix_table = matrix_markdown_table({"rows": rows}) if rows else "—"

    # ======================
    # Каноническая расшифровка по ячейкам (без фантазии ИИ)
    # describe_cell(pot, col) должен возвращать готовый текст
    # ======================
    COL_LABELS_LOCAL = {
        "perception": "Восприятие",
        "motivation": "Мотивация",
    ...
    ROW_TITLES = {
        "1": "1 ряд — ядро, природа, реализация",
        "2": "2 ряд — энергия наполнения и социальное взаимодействие",
        "3": "3 ряд — риски, сливы, что делегировать",
    }

    explain_blocks = []
    for r in rows:
        row_id = str(r.get("row", "")).strip() or "—"
        title = ROW_TITLES.get(row_id, f"Ряд {row_id}")
        parts = [f"### {title}"]

        for col in ["perception", "motivation", "instrument"]:
            pot = r.get(col)
            if pot and str(pot).strip() and pot != "—":
                # describe_cell обязан быть “каноном”
                # т.е. текст строго соответствует потенциалу и колонке
                parts.append(f"**{pot} — {COL_LABELS_LOCAL.get(col, col)}**\n{describe_cell(pot, col)}")

        explain_blocks.append("\n\n".join(parts))

    matrix_explain = "\n\n".join(explain_blocks) if explain_blocks else "—"

    # ======================
    # Гипотеза “что тормозит” (без заданий)
    # ======================
    top3 = sorted(scores.items(), key=lambda x: float(x[1]), reverse=True)[:3]
    top_names = [p for p, v in top3 if float(v) > 0]

    # Смысловая гипотеза: строим из нескольких источников
    hyp_lines = []

    if trap != "—":
        hyp_lines.append(f"Точка торможения чаще всего сидит в паттерне: **{trap.lower()}** — из-за него старт откладывается, даже если внутри всё уже готово.")
    if hate != "—":
        hyp_lines.append(f"Дополнительный конфликт — сопротивление к зоне: **{hate.lower()}**. Это не “лень”, а реальная несовместимость с частью процесса.")
    if avoid != "—" and avoid != "—":
        hyp_lines.append("Когда такие задачи накапливаются, мозг включает защиту: откладывание/переключение/самокритику — чтобы не проживать напряжение.")
    if stress != "—":
        hyp_lines.append(f"Под давлением твой автоматический сценарий: **{stress.lower()}** — и именно он часто ломает темп и уверенность.")

    if not hyp_lines and top_names:
        hyp_lines.append("По структуре ответов видно внутренний конфликт: ты одновременно хочешь быстрый результат и безопасность/идеальность — и это сталкивает тебя в стопор на старте.")
    if not hyp_lines:
        hyp_lines.append("По ответам видно, что запрос зрелый, но не хватает пары точных примеров, чтобы зафиксировать главный блок (он есть, просто ещё не назван).")

    hypothesis = "\n".join([f"- {x}" for x in hyp_lines])

    # ======================
    # Финальный текст
    # ======================
    return f"""
**{name}, твой предварительный отчёт по запросу: “{request}”**

## Твоя матрица потенциалов 3×3 (предварительно)
{matrix_table}
...
## Где сейчас сливается энергия
- **слив:** {leak}
- **что откладываешь:** {avoid}
- **самое нелюбимое:** {hate}

## Расшифровка матрицы (по камням, канон)
{matrix_explain}

## Главная гипотеза: почему пока не получается
{hypothesis}

Если ты хочешь **точность и глубину**, то полный разбор с мастером даёт то, чего не даст автоматический отчёт:
- точное чтение конфликта между рядами/колонками
- проверку “смещений” (где ты живёшь не своей доминантой)
- ясную интерпретацию твоего запроса через матрицу и жизненные примеры

**Формат:** полный мастер-отчёт + 60 минут консультации по твоей матрице.
""".strip()
